...
function SignupForm() {
  ...
  const handleVerify = async (e) => {
    ...
    try {
      ...
      setDone(true);
      login({ token: res.data.token, user: res.data.user }, true);
    ...
    return (
      <form onSubmit={handleVerify} className="flex flex-col gap-4">
        <div className="text-center text-sm text-cyan-50 mb-2">
          ...
          <p className="font-medium text-lg mb-1">Check your email</p>
        ...
        <button
          type="submit"
          disabled={isVerifying || done || !verificationCode}
        ...
        <button
          ...
          className="text-sm text-cyan-300/80 hover:text-cyan-200 transition-colors disabled:opacity-50 disabled:cursor-not-allowed mt-2"
        >
          {resendCooldown > 0
